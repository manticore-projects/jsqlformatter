...
SELECT /*+ PARALLEL */ cfe.id_account_seq.nextval
    , a.code
    , a.id_currency
    , a.id_fee_type
    , current_date
...
FROM  ( SELECT *
            FROM  ( SELECT DISTINCT c.code code
                            , d.id_currency
                            , NULL id_fee_type
...
SELECT /*+ PARALLEL */ cfe.id_collateral_ref.nextval
    , id_collateral
FROM  ( SELECT DISTINCT a.id_collateral
...
SELECT /*+ PARALLEL */ cfe.id_counter_party_ref.nextval
    , id_counter_party
FROM  ( SELECT DISTINCT a.id_counter_party
            FROM cfe.collateral a
